...
from baby_pytorch.nn import Sigmoid
from baby_pytorch.tensor import Tensor


def test_sigmoid_layer_has_no_parameters_or_weights():
    layer = Sigmoid()

    assert layer.parameters() == []
    assert layer.save_weights() == []
    assert layer.load_weights([]) is None
    assert repr(layer) == "Sigmoid"


def test_sigmoid_layer_values_and_gradients_match_pytorch():
    input_data = np.array([[-20.0, -0.5, 0.0], [0.25, 1.0, 20.0]])
    ...
    torch_result = torch.sigmoid(torch_input)
...
def test_sigmoid_layer_detaches_its_output_during_inference():
    tensor = Tensor([[-1.0, 0.0, 1.0]], requires_grad=True)

    result = Sigmoid()(tensor, training=False)

    expected = 1.0 / (1.0 + np.exp(-tensor.data))
